...
class ConfigUI:
    ...
    def _create_widgets(self):
        ...
        self._add_entry(print_frame, "数据监听端口:", "9111", 1, key="data_listen_port")
        self._add_entry(print_frame, "转发目标地址:", "127.0.0.1", 2, key="forward_target_addr")
        self._add_entry(print_frame, "转发目标端口:", "9100", 3, key="forward_target_port")
        self._add_entry(print_frame, "打印长度设置:", "100000", 4, unit="mm", key="print_length")
        self._add_entry(print_frame, "打印宽度设置:", "1800", 5, unit="mm", key="print_width")
        self._add_entry(print_frame, "缓冲字节设置:", "10240", 6, unit="KB", key="buffer_size")
        ...
        self._add_entry(log_frame, "日志存储上限:", "10240", 1, unit="MB", key="log_limit")
        ...
        self._add_entry(cam_settings_frame, "相机缓冲区数:", "10", 0, key="cam_buffer_count")
        self._add_entry(cam_settings_frame, "数据接收地址:", "127.0.0.1", 1, key="data_recv_addr")
        self._add_entry(cam_settings_frame, "数据接收端口:", "9120", 2, key="data_recv_port")
        self._add_entry(cam_settings_frame, "采集图像长度:", "1000", 3, unit="cm", key="capture_img_len")
        self._add_browse_entry(cam_settings_frame, "标定文件选择:", 4, is_file=True, default_val=r"D:\vsDriverbox\calib.yaml", key="calib_file")
        ...
        self._add_entry(stitch_frame, "拼接左侧基准:", "2461790", 0, key="stitch_left_ref")
        self._add_entry(stitch_frame, "拼接右侧基准:", "29057700", 1, key="stitch_right_ref")
        self._add_entry(stitch_frame, "图像拼接偏移:", "629", 2, unit="pix", key="img_stitch_offset")
        self._add_entry(stitch_frame, "画布开始位置:", "7906", 3, key="canvas_start_pos")
        self._add_entry(stitch_frame, "画布结束位置:", "21259", 4, key="canvas_end_pos")
        self._add_entry(stitch_frame, "重叠偏移像素:", "500", 5, key="overlap_offset_pix")
        self._add_entry(stitch_frame, "DPI:", "300", 6, key="dpi")
        ...
        self._add_entry(board_frame, "通讯端口设置:", "502", 1, key="board_comm_port")
        self._add_entry(board_frame, "通讯超时设置:", "3000", 2, unit="ms", key="board_comm_timeout")
        self._add_entry(board_frame, "通讯重试次数:", "3", 3, unit="次", key="board_comm_retry")
        ...
        self._add_entry(light_frame, "波特率设置:", "19200", 1, key="light_baudrate")
        self._add_entry(light_frame, "通讯超时设置:", "1000", 2, unit="ms", key="light_comm_timeout")
        self._add_entry(light_frame, "灯光亮度设置:", "100", 3, key="light_brightness")
    ...
    def _add_entry(self, parent, label_text, default_val, row, unit="", key=None):
    ...
    def _add_browse_entry(self, parent, label_text, row, is_file=False, default_val="", show_status=True, key=None):
